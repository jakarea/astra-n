...
export default function RegisterPage() {
  ...
  const validateForm = () => {
    ...
    const commonDomains = ['gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com', 'libero.it', 'virgilio.it']
    const emailDomain = formData.email.split('@')[1]?.toLowerCase()
    if (emailDomain && !commonDomains.some(domain => emailDomain.includes(domain.split('.')[0]))) {
      // Don't block, just warn - this is just extra validation
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.needsEmailConfirmation) {
          ...
          // If no email confirmation required, redirect to orders
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {success && (
                <Alert className="border-teal-600 bg-teal-900/20">
                  <CheckCircle className="h-4 w-4 text-teal-500" />
                  <AlertDescription className="text-teal-100">{success}</AlertDescription>
                </Alert>
              )}
